...
char PVstring[12];
char SVstring[12];
int8_t speed = 11, rotation = 6, interval = 1, timePeriod = 1;
char rotationString[20], intervalString[20], timePeriodString[20], speedString[20];
...
void userInterface(void const * argument)
{
  ...
  while (1)
  {
	...
	if (chooseMenu == 0){
		...
		ssd1306_SetCursor(5, 20);
		ssd1306_WriteString(speedString, Font_7x10, White);

		ssd1306_Line(5, 30, 40, 30, White);
		ssd1306_UpdateScreen();
		ssd1306_Line(5, 30, 40, 30, Black);
		ssd1306_UpdateScreen();
	}
  }
  /* USER CODE END 5 */
